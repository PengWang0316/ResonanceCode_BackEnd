...
module.exports = (req, res) => {
  ...
  return mongodb.getJournalList(queryObject).then(result =>
    res.json(result[0].journal_entries
      .sort((previous, next) => new Date(next.date).getTime() - new Date(previous.date).getTime()))).catch(err => logger.error('/fetchJournals', err));
};
